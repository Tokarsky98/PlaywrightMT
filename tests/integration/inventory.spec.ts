...
test.describe('Inventory - Cart Operations', () => {
    ...
    test(
        'should add and remove items from the cart',
        { tag: ['@logged'] },
        async ({ inventoryPage }) => {
            ...
            const addedItems = await inventorySection.actionOnCart(
                'add',
                twoItems,
            );
            ...
            await inventoryView.inventorySection.actionOnCart(
                'remove',
                twoItems,
            );
            ...
            expect(
                await cartViewAgain.cartListSection.getNamesOfItems(),
            ).toEqual([]);
        },
    );

    test(
        'should check the total price of added items',
        { tag: ['@logged'] },
        async ({ inventoryPage }) => {
            ...
            const totalPrice =
                await inventorySection.calculateTotalPriceWithoutTax(
                    addedItems.names,
                );

            const checkoutStepTwoView =
                await inventoryPage.goToCheckoutStepTwo();
            await expect(
    ...
    test.describe('Edge cases', () => {
        test(
            'should maintain cart state after browser refresh',
            { tag: ['@logged'] },
            async ({ inventoryPage, page }) => {
                ...
                const cartView = await header.clickCartIcon();
                const cartItems =
                    await cartView.cartListSection.getNamesOfItems();
...
test.describe('Inventory - Sorting Functionality', () => {
    test(
        'should check the sorting of items',
        { tag: ['@logged'] },
        async ({ inventoryPage }) => {
            ...
            for (const { filter, orderBy, expectedOrder } of sorting) {
                ...
                const presentOrder =
                    await inventorySection.getItemsOrder(orderBy);
                expect(
                    presentOrder,
