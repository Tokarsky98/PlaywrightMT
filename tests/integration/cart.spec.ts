...
test.describe('Cart Management', () => {
    test('should display item added via fixture in cart @logged', async ({
        item,
        cartPage,
    }) => {
        ...
        const actualItems = await cartPage.cartListSection.getNamesOfItems();
    ...
    test.describe('Single item in cart', () => {
        ...
        test('should display overridden fixture item in cart @logged', async ({
            items,
            cartPage,
        }) => {
            const actualItems =
                await cartPage.cartListSection.getNamesOfItems();
            expect(actualItems, 'Cart should contain overridden item').toEqual([
                items[0].name,
            ]);
        });
    });
    ...
    test.describe('Multiple items in cart', () => {
        ...
        test('should display all items added via fixture @logged', async ({
            items,
            cartPage,
        }) => {
            const actualItems =
